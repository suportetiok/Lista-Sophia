...
window.abrirListaCompras = async function() {
  ...
  conteudo.innerHTML = comprados.length === 0 
...
window.abrirLogs = async function() {
  ...
  try {
    ...
    conteudo.innerHTML = logs.length === 0 
...
function renderGifts() {
  ...
  giftsGrid.innerHTML = giftsData.length === 0 
    ? "<p class='text-center col-span-full bg-white/80 p-4 rounded'>Nenhum item cadastrado</p>" 
    : giftsData.map(item => `
